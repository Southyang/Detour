...
from model.net import DetourPolicy_nodiffusion
from model.ppo import ppo_update_stage1, ppo_update_stage1_improved, generate_train_data
...
from env_stage1_nodiffusion import Environment
...
def run(comm, env:Environment, policy, policy_path, action_bound, optimizer):
    ...
    while id < MAX_EPISODES:
        ...
        if IMPROVED:
            state = [laser_stack, camera_obs, goal, speed, orientation]
        else:
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            if IMPROVED:
                v, a, logprob, scaled_action = generate_action_improved(env=env, state_list=state_list,
            ...
            if IMPROVED:
                state_next = [laser_stack, img_next, goal_next, speed_next, orientation_next]
            else:
            ...
            if global_step % HORIZON == 0:
                ...
                if IMPROVED:
                    last_v, _, _, _ = generate_action_improved(env=env, state_list=state_next_list, policy=policy,
            ...
            if env.index == 0 and IF_TRAIN:
                ...
                if len(buff) > HORIZON - 1:
                    if IMPROVED:
                        ...
                        s_batch, img_batch, goal_batch, speed_batch, orientation_batch, a_batch, r_batch, d_batch, l_batch, v_batch = \
                            transform_buffer_improved(buff=buff)
                        ...
                        memory = (s_batch, img_batch, goal_batch, speed_batch, orientation_batch, a_batch, l_batch, t_batch, v_batch, r_batch, advs_batch)
                    
                        actor_loss, critic_loss, total_loss = ppo_update_stage1_improved(policy=policy, optimizer=optimizer, batch_size=BATCH_SIZE, memory=memory,
...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        if IMPROVED :
            print("Use DetourPolicy_nodiffusion")
            policy = DetourPolicy_nodiffusion(frames=LASER_HIST, action_space=2)
        else :
            print("Use test")
