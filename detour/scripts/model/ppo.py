...
dirname = now.strftime("Stage2_%b%d_%H%M")  # 每次启动的时候，改Stage1或者2
dirname = './log/'+dirname

if not os.path.exists(dirname):
    os.makedirs(dirname, exist_ok=True)
ppo_file = dirname + '/ppo.log'
...
def transform_buffer_improved(buff):
    s_batch, img_batch, goal_batch, speed_batch, orientation_batch, a_batch, r_batch, d_batch, l_batch, \
    v_batch = [], [], [], [], [], [], [], [], [], []
    s_temp, img_temp, goal_temp, speed_temp, orientation_temp = [], [], [], [], []

    for e in buff:
        for state in e[0]:
            s_temp.append(state[0])
            img_temp.append(state[1])
            goal_temp.append(state[2])
            speed_temp.append(state[3])
            orientation_temp.append(state[4])
        s_batch.append(s_temp)
        img_batch.append(img_temp)
        goal_batch.append(goal_temp)
        speed_batch.append(speed_temp)
        orientation_batch.append(orientation_temp)
        s_temp = []
        img_temp = []
        goal_temp = []
        speed_temp = []
    ...
    goal_batch = np.asarray(goal_batch)
    speed_batch = np.asarray(speed_batch)
    orientation_batch = np.asarray(orientation_batch)
    a_batch = np.asarray(a_batch)
    r_batch = np.asarray(r_batch)
    d_batch = np.asarray(d_batch)
    l_batch = np.asarray(l_batch)
    ...
    return s_batch, img_batch, goal_batch, speed_batch, orientation_batch, a_batch, r_batch, d_batch, l_batch, v_batch

def transform_buffer_diffusion(buff):
    s_batch, img_batch, pre_batch, goal_batch, speed_batch, orientation_batch, a_batch, r_batch, d_batch, l_batch, \
    v_batch = [], [], [], [], [], [], [], [], [], [], []
    s_temp, img_temp, pre_temp, goal_temp, speed_temp, orientation_temp = [], [], [], [], [], []

    for e in buff:
        for state in e[0]:
            s_temp.append(state[0])
            img_temp.append(state[1])
            pre_temp.append(state[2])
            goal_temp.append(state[3])
            speed_temp.append(state[4])
            orientation_temp.append(state[5])
        s_batch.append(s_temp)
        img_batch.append(img_temp)
        pre_batch.append(pre_temp)
        goal_batch.append(goal_temp)
        speed_batch.append(speed_temp)
        orientation_batch.append(orientation_temp)
        s_temp = []
    ...
    return s_batch, img_batch, pre_batch, goal_batch, speed_batch, orientation_batch, a_batch, r_batch, d_batch, l_batch, v_batch
...
def generate_action_improved(env, state_list, policy, action_bound):
    if env.index == 0:
        s_list, img_list, goal_list, speed_list, orientaion_list = [], [], [], [], []
        for i in state_list:
            s_list.append(i[0])
            img_list.append(i[1])
            goal_list.append(i[2])
            speed_list.append(i[3])
            orientaion_list.append(i[4])

        s_list = np.asarray(s_list)
        img_list = np.asarray(img_list)
        goal_list = np.asarray(goal_list)
        speed_list = np.asarray(speed_list)
        orientaion_list = np.asarray(orientaion_list)

        s_list = Variable(torch.from_numpy(s_list)).float().cuda()
        # torch.Size([3, 45, 80, 3]), (env_num,width,length,channel)
        img_list = Variable(torch.from_numpy(img_list)).float().cuda()
        goal_list = Variable(torch.from_numpy(goal_list)).float().cuda()
        speed_list = Variable(torch.from_numpy(speed_list)).float().cuda()
        orientaion_list = Variable(torch.from_numpy(orientaion_list)).float().cuda()

        # 转换为shape为 torch.Size([3, 3, 45, 80]) (env_num,channel,width,length)
        img_list = img_list.permute(0, 3, 1, 2)

        v, a, logprob, mean = policy(s_list, img_list, goal_list, speed_list, orientaion_list)
        v, a, logprob = v.data.cpu().numpy(), a.data.cpu().numpy(), logprob.data.cpu().numpy()
        scaled_action = np.clip(a, a_min=action_bound[0], a_max=action_bound[1])
    else:
        v = None
...
def generate_action_diffusion(env, state_list, policy, action_bound):
    if env.index == 0:
        s_list, img_list, pre_list, goal_list, speed_list, orientaion_list = [], [], [], [], [], []
        for i in state_list:
            s_list.append(i[0])
            img_list.append(i[1])
            pre_list.append(i[2])
            goal_list.append(i[3])
            speed_list.append(i[4])
            orientaion_list.append(i[5])
        ...
        # torch.Size([3, 45, 80, 3]), (env_num,width,length,channel)
        img_list = Variable(torch.from_numpy(img_list)).float().cuda()
        pre_list = Variable(torch.from_numpy(pre_list)).float().cuda()
        goal_list = Variable(torch.from_numpy(goal_list)).float().cuda()
        speed_list = Variable(torch.from_numpy(speed_list)).float().cuda()
        orientaion_list = Variable(torch.from_numpy(orientaion_list)).float().cuda()
        ...
        # 转换为shape为 torch.Size([3, 3, 45, 80]) (env_num,channel,width,length)
        img_list = img_list.permute(0, 3, 1, 2)
        pre_list = pre_list.permute(0, 3, 1, 2)

        v, a, logprob, mean = policy(s_list, img_list, pre_list, goal_list, speed_list, orientaion_list)
        v, a, logprob = v.data.cpu().numpy(), a.data.cpu().numpy(), logprob.data.cpu().numpy()
        scaled_action = np.clip(a, a_min=action_bound[0], a_max=action_bound[1])
    else:
        v = None
...
def ppo_update_stage1(policy, optimizer, batch_size, memory, epoch,
    ...
    for update in range(epoch):
        ...
        for i, index in enumerate(sampler):
            ...
            actor_loss += info_p_loss
            critic_loss += info_v_loss
            total_loss += loss

    return actor_loss/epoch, critic_loss/epoch, total_loss/epoch

def ppo_update_stage1_improved(policy, optimizer, batch_size, memory, epoch,
               coeff_entropy=0.02, clip_value=0.2,
               num_step=2048, num_env=12, frames=1, obs_size=24, act_size=4):
    obss, imgs, goals, speeds, orientations, actions, logprobs, targets, values, rewards, advs = memory
    ...
    # cv2.imshow('Image', imgs[0][0])
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()

    # imgs = imgs.reshape((num_step*num_env, 3, 80, 45))
    imgs = imgs.reshape((num_step * num_env, 3, 45, 80))

    goals = goals.reshape((num_step*num_env, 2))
    speeds = speeds.reshape((num_step*num_env, 2))
    orientations = orientations.reshape((num_step*num_env, 4))
    actions = actions.reshape(num_step*num_env, act_size)
    logprobs = logprobs.reshape(num_step*num_env, 1)
    advs = advs.reshape(num_step*num_env, 1)
    targets = targets.reshape(num_step*num_env, 1)
    ...
    for update in range(epoch):
        ...
        for i, index in enumerate(sampler):
            sampled_obs = Variable(torch.from_numpy(obss[index])).float().cuda()
            sampled_imgs = Variable(torch.from_numpy(imgs[index])).float().cuda()
            sampled_goals = Variable(torch.from_numpy(goals[index])).float().cuda()
            sampled_speeds = Variable(torch.from_numpy(speeds[index])).float().cuda()
            sampled_orientations = Variable(torch.from_numpy(orientations[index])).float().cuda()
            ...
            new_value, new_logprob, dist_entropy = policy.evaluate_actions(sampled_obs, sampled_imgs, sampled_goals, sampled_speeds, sampled_orientations, sampled_actions)
            ...
            loss = policy_loss + 20 * value_loss - coeff_entropy * dist_entropy
            # loss = policy_loss + 0.01*value_loss - coeff_entropy * dist_entropy
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            info_p_loss, info_v_loss, info_entropy = float(policy_loss.detach().cpu().numpy()), \
...
def ppo_update_stage1_diffusion(policy, optimizer, batch_size, memory, epoch,
               coeff_entropy=0.02, clip_value=0.2,
               num_step=2048, num_env=12, frames=1, obs_size=24, act_size=4):
    obss, imgs, pres, goals, speeds, orientations, actions, logprobs, targets, values, rewards, advs = memory
    ...
    # imgs = imgs.reshape((num_step*num_env, 3, 80, 45))
    imgs = imgs.reshape((num_step * num_env, 3, 45, 80))
    pres = pres.reshape((num_step * num_env, 3, 45, 80))
    ...
    for update in range(epoch):
        ...
        for i, index in enumerate(sampler):
            sampled_obs = Variable(torch.from_numpy(obss[index])).float().cuda()
            sampled_imgs = Variable(torch.from_numpy(imgs[index])).float().cuda()
            sampled_pres = Variable(torch.from_numpy(pres[index])).float().cuda()
            sampled_goals = Variable(torch.from_numpy(goals[index])).float().cuda()
            sampled_speeds = Variable(torch.from_numpy(speeds[index])).float().cuda()
            sampled_orientations = Variable(torch.from_numpy(orientations[index])).float().cuda()
            ...
            new_value, new_logprob, dist_entropy = policy.evaluate_actions(sampled_obs, sampled_imgs, sampled_pres, sampled_goals, sampled_speeds, sampled_orientations, sampled_actions)
...
def ppo_update_stage2_improved(policy, optimizer, batch_size, memory, filter_index, epoch,
               coeff_entropy=0.02, clip_value=0.2,
               num_step=2048, num_env=12, frames=1, obs_size=24, act_size=4):
    obss, imgs, goals, speeds, orientations, actions, logprobs, targets, values, rewards, advs = memory
    ...
    obss = np.delete(obss, filter_index, 0)
    imgs = np.delete(imgs, filter_index, 0)
    goals = np.delete(goals, filter_index, 0)
    speeds = np.delete(speeds, filter_index, 0)
    orientations = np.delete(orientations, filter_index, 0)
    actions = np.delete(actions, filter_index, 0)
    logprobs  = np.delete(logprobs, filter_index, 0)
    advs = np.delete(advs, filter_index, 0)
    targets = np.delete(targets, filter_index, 0)
...
def ppo_update_stage2_diffusion(policy, optimizer, batch_size, memory, filter_index, epoch,
               coeff_entropy=0.02, clip_value=0.2,
               num_step=2048, num_env=12, frames=1, obs_size=24, act_size=4):
    obss, imgs, pres, goals, speeds, orientations, actions, logprobs, targets, values, rewards, advs = memory
    ...
    obss = obss.reshape((num_step*num_env, frames, obs_size))
    imgs = imgs.reshape((num_step*num_env, 3, 80, 45))
    pres = pres.reshape((num_step * num_env, 3, 80, 45))
    # imgs = imgs.reshape((num_step * num_env, 3, 45, 80))
    goals = goals.reshape((num_step*num_env, 2))
    speeds = speeds.reshape((num_step*num_env, 2))
    orientations = orientations.reshape((num_step*num_env, 4))
    ...
    obss = np.delete(obss, filter_index, 0)
    imgs = np.delete(imgs, filter_index, 0)
    pres = np.delete(pres, filter_index, 0)
    goals = np.delete(goals, filter_index, 0)
    speeds = np.delete(speeds, filter_index, 0)
    orientations = np.delete(orientations, filter_index, 0)
    actions = np.delete(actions, filter_index, 0)
