...
from model.eval import ImagePredictor

MAP_SIZE = 10.   # 2倍为地图长宽，stage2里面并没有用到
...
class Environment:
    ...
    def reset_pose(self, init_pose):
        random_pose = init_pose
        rospy.sleep(0.01)
    ...
    def generate_goal_point(self, goal_point):
        [x_g, y_g] = [goal_point[0], goal_point[1]]
    ...
    def generate_random_goal(self):
        ...
        while (dis_origin > MAP_SIZE or dis_goal > MAP_SIZE or dis_goal < MAP_SIZE/3) and not rospy.is_shutdown():
            x = np.random.uniform(-MAP_SIZE, MAP_SIZE)
            y = np.random.uniform(-MAP_SIZE, MAP_SIZE)
    ...
    def tensor_to_numpy(self, tensor_image):
        ...
        """
        # 移动到 CPU 并去掉批次维度，转为 NumPy 数组
        numpy_image = tensor_image.squeeze(0).detach().cpu().numpy()

        # 变换维度从 (channels, height, width) 到 (height, width, channels)
        numpy_image = numpy_image.transpose(1, 2, 0)

        # 反归一化，将 [-1, 1] 范围转换为 [0, 255]
        numpy_image = (numpy_image * 0.5 + 0.5) * 255
        numpy_image = numpy_image.astype(np.uint8)

        return numpy_image

    def get_observation_predict(self):
        compressed_image = np.frombuffer(self.image, np.uint8)
        image_obs = cv2.imdecode(compressed_image, cv2.IMREAD_COLOR)

        predicted_image = self.predictor.predict_image(image_obs)

        predicted_image = self.tensor_to_numpy(predicted_image)
    ...
    def get_reward_and_terminate2(self, t):
        # 阶段二奖励函数,加大时间惩罚，增加持续性倾斜惩罚，旋转惩罚
        reward_g = 0  # 靠近目标奖励
        reward_g_extra = 0 # 额外靠近目标奖励
        reward_c = 0  # 碰撞惩罚
        reward_c_extra = 0 # 额外碰撞惩罚
        reward_w = 0  # 角速度惩罚
        reward_t = 0  # 倾斜惩罚
        reward_s = 0  # 时间步惩罚
        result = 0
        ...
        if self.distance <= self.goal_size:
            ...
            if self.pre_distance > self.distance and self.distance <= 2:
                reward_g_extra = 2 - self.distance
        ...
        if is_crash:
            ...
            if self.pre_min_collision_dist > self.min_collision_dist: # 靠近障碍物惩罚
                reward_c = (self.pre_min_collision_dist - self.min_collision_dist) * 2
            if self.min_collision_dist <= 1.5: # 额外碰撞惩罚
                reward_c_extra = 1.5 - self.min_collision_dist
        ...
        if IMPROVED_REWARD:
            reward = reward_g + reward_g_extra - reward_c - reward_c_extra - reward_t - reward_s - reward_w
        else:
            reward = reward_g - reward_c - reward_w

        if reward < 0:
            reward = max(reward, -MAX_PUNISH * 2)
